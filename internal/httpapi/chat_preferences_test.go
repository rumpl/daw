...
func TestOpenChatRestoresPreferencesAndBindsDefaultsToSession(t *testing.T) {
	...
	first.openChat(recorder, httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/api/chats", http.NoBody), "ws", "", "", nil, "", "", "", "", false)
	...
	recorder = httptest.NewRecorder()
	second.openChat(recorder, httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/api/chats/resume", http.NoBody), "ws", ref.SessionID, "", nil, "", "", "", "", false)
	if recorder.Code != 201 {
...
func TestGlobalToolFilterUpdatesLiveChats(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	s.openChat(recorder, httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/api/chats", http.NoBody), "ws", "", "", nil, "", "", "", "", false)
