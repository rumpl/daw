...
export function useChat(chatId: string | null) {
  ...
  const prepareChat = useCallback(async (nextChatId: string) => {
    if (cache.current.has(nextChatId)) return;
    const snap = await api.snapshot(nextChatId);
    cache.current.set(nextChatId, applySnapshot(snap));
  }, []);
  ...
  return { state, connection, prepareChat, resnapshot, setState };
}
